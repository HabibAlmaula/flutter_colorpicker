...
class _SliderLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    positionChild(gestureContainer, Offset(0.0, size.height * 0.4));
  }
...
class ThumbPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var newOffset = Offset(0.0, size.height * 0.5);
    var thumbHeight = height * 0.6;
    final RRect thumbRect = RRect.fromRectAndRadius(
      Rect.fromCenter(center: newOffset, width: width, height: thumbHeight),
      // Adjust the size of the rounded thumb
      const Radius.circular(radius),
    );

    final squarePaint = Paint()
      ..color = Colors.white // Customize thumb color
      ..style = PaintingStyle.stroke
      ..strokeWidth = 5.0;

    final path = Path()..addRRect(thumbRect);
    ...
    //   true,
    // );
    canvas.drawShadow(path, Colors.grey.withAlpha(50), 8.0, false);

    canvas.drawRRect(thumbRect, squarePaint);
    // canvas.drawCircle(
...
class ColorPickerSlider extends StatelessWidget {
  const ColorPickerSlider(
    ...
    this.fullThumbColor = false,
    this.sliderHeight = 100.0,
    this.thumbSliderWidth = 12.0,
  }) : super(key: key);

  final double sliderHeight;
  final double thumbSliderWidth;
  final TrackType trackType;
  final HSVColor hsvColor;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints box) {
      ...
      double thumbWidth = 45.0; // Adjust this value based on your thumb size
      double thumbOffset =
          thumbWidth / 2.0; // Half of the thumb width for centering

      Color thumbColor;
      switch (trackType) {
        case TrackType.hue:
          thumbOffset += (box.maxWidth - thumbWidth) * hsvColor.hue / 360;
          thumbOffset = thumbOffset.clamp(
              thumbWidth / 2.0, box.maxWidth - thumbWidth / 2.0);
